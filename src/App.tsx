...
declare global {
	interface Window {
		umami: {
			track: (...args: [void] | [object] | [string] | [string, object]) => void;
			identify: (...args: [string] | [string, object] | [object]) => void;
		};
	}
}
...
function App() {
				...
				return prevCats.map((cat) => {
					...
					if (timeSinceLastStateChange > 2000 + Math.random() * 3000) {
						...
						if (cat.state === "running" && actionRoll < 0.3) {
							...
							(cat.state === "sleeping" || cat.state === "sitting") &&
							actionRoll < 0.6
						) {
	...
	useEffect(() => {
		...
		setCats((prevCats) => {
			...
			if (followCursor && prevCats.length > 0) {
				...
				const eligibleCats = prevCats.filter(
					(cat) => cat.state !== "dead" && cat.state !== "angry"
				);
				...
				if (eligibleCats.length > 0) {
					...
					const nearestCat = eligibleCats.reduce((nearest, current) => {
						const nearestDist = Math.hypot(
							nearest.position.x - mousePosition.x,
							nearest.position.y - mousePosition.y
						);
						const currentDist = Math.hypot(
							current.position.x - mousePosition.x,
							current.position.y - mousePosition.y
						);
						return currentDist < nearestDist ? current : nearest;
					});

					// Update that cat to follow cursor
					const nearestCatIndex = catsToUpdate.findIndex(
						(cat) => cat.id === nearestCat.id
					);
			...
			if (uneatenFood.length > 0) {
				...
				const cursorFollowerCatId = catsToUpdate.find(
					(cat) =>
						cat.targetPosition.x === mousePosition.x &&
						cat.targetPosition.y === mousePosition.y
				)?.id;

				catsToUpdate.forEach((cat, index) => {
					if (
						cat.id !== cursorFollowerCatId &&
						cat.state !== "dead" &&
						cat.state !== "angry"
					) {
						// Check if this cat already has a food target
						if (cat.foodTargetId !== null) {
							const existingTarget = updatedFood.find(
								(f) => f.id === cat.foodTargetId && !f.eaten
							);

							if (existingTarget) {
								// Still targeting same food
								updatedFood = updatedFood.map((f) =>
									f.id === existingTarget.id
										? { ...f, targetedBy: [...f.targetedBy, cat.id] }
										: f
								);
						...
						const availableFood = uneatenFood
							...
							.sort((a, b) => {
								...
								const distA = Math.hypot(
									cat.position.x - a.position.x,
									cat.position.y - a.position.y
								);
								const distB = Math.hypot(
									cat.position.x - b.position.x,
									cat.position.y - b.position.y
								);
								return distA - distB;
							});
	...
	const handleAddCat = useCallback(() => {
		...
		// Track the new cat with Umami
		window.umami?.track("cat-added");
	...
	const handleRandomAddCat = useCallback(() => {
		...
		window.umami?.track("cat-added-random", {
			count,
		});
	}, [windowDimensions]);
	...
	const handleAddFood = useCallback(() => {
		...
		// Track the new food with Umami
		window.umami?.track("food-added", {
			count: foodCount,
		});
	...
	const handleRemoveAllCats = useCallback(() => {
		setCats([]);
		setNumCats(0);
		window.umami?.track("cats-all-removed");
	}, []);

	// Remove all food
	const handleRemoveAllFood = useCallback(() => {
		setFood([]);
		window.umami?.track("food-all-removed");
	}, []);
	...
	return (
		...
		>
			...
			>
				<p className="text-sm">
					Anonymized interaction data (without IP data) is automatically sent to a
					self-hosted <span className="font-semibold">Umami</span> server for analytics.
				</p>
			</footer>
		</div>
	);
